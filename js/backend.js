...
pool.on("connect", async () => {
  ...
  try {
    await pool.query(
      "ALTER TABLE IF EXISTS cart DROP CONSTRAINT IF EXISTS cart_product_id_fkey",
    );
...
const createFavoritesTable = require("./models/createFavorites");
const createCompareTable = require("./models/createCompare");
const createUsersTable = require("./models/createUsers");
...
pool.on("connect", async () => {
  try {
    ...
    await createFavoritesTable(pool);
    await createCompareTable(pool);
    console.log(
      "✅ All tables ensured (users, orders, order_items, favorites, compare)",
    );

    // Migration for favorites is handled inside createFavoritesTable (createFavorites.js).
    // No extra migration needed here.
  } catch (e) {
    console.error("⚠️ Error creating tables:", e.message);
  }
});